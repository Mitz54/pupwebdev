...
function createtable()
{
  ...
  $sql = "select *,borrower.name as borrowname,item.name as itemname from borroweditems 
		  ...
		  inner join item on iteminfo.itemID_FK = item.itemID
		  inner join borrowingdetails on borrowingdetails.borrowingDetailsID = borroweditems.borrowingDetailsID_FK
		  inner join borrower on borrowingdetails.borrowerID_FK = borrower.borrowerID  where verified = 0 and borrower.name like concat('%','".$search."','%') group by borrowingDetailsID_FK;";
  $result = $conn->query($sql);
  ...
  if ($result->num_rows > 0) 
  {
      ...
      while($row = $result->fetch_assoc()) 
      {
        ...
        echo '<center>
        		<td>
              		<button type="button" id="'.$row['borrowingDetailsID'].'" class="btn btn-sm btn-secondary approve_pending_item"><i class="fas fa-check"></i></button>
              		<button type="button" id="'.$row['borrowingDetailsID'].'" class="btn btn-sm btn-secondary delete_pending_item" data-toggle="modal" data-target="#actionDeleteItemModal"><i class="fas fa-times"></i></button>
              	</td>
              </center>';
        echo "</tr>"; 
      }
